...
if command -v pv &>/dev/null; then
  ...
  unzip -o "firmware.zip" | pv -f -i 2 -l -s "$TOTAL_FILES" >/dev/null || true
else
...
if command -v pv &>/dev/null; then
  pv -f -i 2 "$AP_FILE" | tar -xf - 2>/dev/null || true
else
  ...
  for PART in $USER_PARTITIONS; do
    for SUFFIX in "" "_a" "_b"; do
      ...
      if [ -n "$FILE" ] && [ -s "$FILE" ]; then
        ...
        if command -v pv &>/dev/null; then
          pv -f -i 2 "$FILE" | lz4 -d - "${FILE%.lz4}" 2>/dev/null || true
        else
        ...
        if [ -s "$FILE" ]; then
          if command -v pv &>/dev/null; then
            if pv -f -i 2 "$FILE" | xz -${COMP_LEVEL} -T0 > "processed/${PART}${SUFFIX}.img.xz" 2>/dev/null; then
              echo "        ✓ Compressed to .xz"
            else
              rm -f "processed/${PART}${SUFFIX}.img.xz"
              pv -f -i 2 "$FILE" > "processed/${PART}${SUFFIX}.img"
  ...
  for PART in $USER_PARTITIONS; do
    for SUFFIX in "" "_a" "_b"; do
      if [ -s "super_dump/${PART}${SUFFIX}.img" ]; then
        ...
        if command -v pv &>/dev/null; then
          if pv -f -i 2 "super_dump/${PART}${SUFFIX}.img" | xz -${COMP_LEVEL} -T0 > "processed/${PART}${SUFFIX}.img.xz" 2>/dev/null; then
            echo "        ✓ Compressed to .xz"
          else
            rm -f "processed/${PART}${SUFFIX}.img.xz"
            pv -f -i 2 "super_dump/${PART}${SUFFIX}.img" > "processed/${PART}${SUFFIX}.img"
            echo "        ✓ Moved to processed"
          fi
        else
